...
public abstract class AbsEcgView extends FrameLayout {
  ...
  protected void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    desiredHeight = pixelsPerUnit * 5 * 6;
...
//    //TODO
  ...
  private void drawStandard(Canvas canvas) {
    ...
    float lowH = mHeight / 2 - 5 * pixelsPerUnit;
    float hightH = lowH - mGain * pixelsPerUnit;

    gridMinorPaint.setColor(curveColor);
    canvas.drawLine(0, lowH, 1 * pixelsPerUnit, lowH, gridMinorPaint);
    canvas.drawLine(1 * pixelsPerUnit, lowH, 1 * pixelsPerUnit, hightH, gridMinorPaint);
    canvas.drawLine(1 * pixelsPerUnit, hightH, 6 * pixelsPerUnit, hightH, gridMinorPaint);
    canvas.drawLine(6 * pixelsPerUnit, hightH, 6 * pixelsPerUnit, lowH, gridMinorPaint);
    canvas.drawLine(6 * pixelsPerUnit, lowH, 7 * pixelsPerUnit, lowH, gridMinorPaint);
  ...
  private void initSampleParams(float sampleLength) {
    pixelsPerPoint = pixelsPerUnit / (mSamplerate / sampleLength);
  ...
  protected void convertMV2Yaxis(EcgPoint point) {
    point.y = mHeight / 2.0f - point.mv * pixelsPerUnit * mGain;
  }
